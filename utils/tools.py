...
def transfer_weights(weights_path, model, exclude_head=True, device='cpu'):
    new_state_dict = torch.load(weights_path,  map_location=device)['model_state_dict'] #for sim mtm
    #new_state_dict = torch.load(weights_path, map_location=device) #for patchtst
    matched_layers = 0
    unmatched_layers = []
